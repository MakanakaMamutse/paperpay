...
app.post(
  ...
  async (req: Request, res: Response): Promise<any> => {
    ...
    if (!receiverWalletAddress || !manageUrl || !previousToken) {
      return res.status(400).json({
        error: "Validation failed",
        message: "Please fill in all the required fields",
        received: req.body,
    ...
    try {
      ...
      const response = await instantBuy(client!, {
        receiverWalletAddress,
        manageUrl,
        previousToken
      });
      ...
      console.error("Error processing instant buy:", err);
      return res
        .status(500)
        .json({ error: "Failed to execute instant buy" });
    }
  }
);
